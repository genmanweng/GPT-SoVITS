...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        self.configs: dict = configs.get(
            "custom", deepcopy(self.default_configs[default_config_key])
        )
        ...
        self.languages = (
            self.v2_languages if self.version == "v2" else self.v1_languages
        )

        if (self.t2s_weights_path in [None, ""]) or (
            not os.path.exists(self.t2s_weights_path)
        ):
            self.t2s_weights_path = self.default_configs[default_config_key][
                "t2s_weights_path"
            ]
        ...
        if (self.vits_weights_path in [None, ""]) or (
            ...
            self.vits_weights_path = self.default_configs[default_config_key][
                "vits_weights_path"
            ]
        ...
        if (self.bert_base_path in [None, ""]) or (
            ...
            self.bert_base_path = self.default_configs[default_config_key][
                "bert_base_path"
            ]
        ...
        if (self.cnhuhbert_base_path in [None, ""]) or (
            ...
            self.cnhuhbert_base_path = self.default_configs[default_config_key][
                "cnhuhbert_base_path"
            ]
            print(
...
class TTS:
    ...
    def run(self, inputs: dict):
        ...
        if parallel_infer:
            ...
            self.t2s_model.model.infer_panel = (
                self.t2s_model.model.infer_panel_batch_infer
            )
        else:
            print(i18n("并行推理模式已关闭"))
            self.t2s_model.model.infer_panel = (
                self.t2s_model.model.infer_panel_naive_batched
            )
        ...
        if ref_audio_path in [None, ""] and (
            (self.prompt_cache["prompt_semantic"] is None)
            or (self.prompt_cache["refer_spec"] in [None, []])
        ):
            raise ValueError(
        ...
        aux_ref_audio_paths = (
            aux_ref_audio_paths if aux_ref_audio_paths is not None else []
        )
        paths = set(aux_ref_audio_paths) & set(self.prompt_cache["aux_ref_audio_paths"])
        if not (
            len(list(paths))
            == len(aux_ref_audio_paths)
            == len(self.prompt_cache["aux_ref_audio_paths"])
        ):
        ...
        if not return_fragment:
            ...
            if len(data) == 0:
                yield self.configs.sampling_rate, np.zeros(
                    int(self.configs.sampling_rate), dtype=np.int16
                )
                return
    ...
    def audio_postprocess(
        ...
        write("/workspace/video_file/output.wav", sr, audio)
        # try:
        #     if speed_factor != 1.0:
